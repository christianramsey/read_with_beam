...
load_labels <- function(fn) {
  print(filename)
  return(read.csv(fn, skipNul = TRUE))
  return(labels)
}
...
define_label_intervals <- function(loaded_labels) {
  ...
  loaded_labels$intervals <- lubridate::interval(trajectory_labels$interval_start, trajectory_labels$interval_end )
...
save_visualise_trajectories <- function(trajectories, labels, filename) {
  write.csv(trajectories, filename)
...
lab_file = list.files(pattern="*.csv")



for (filename in head(files)) {
  ...
  save_visualise_trajectories(tjs, tj_labels, filename)
